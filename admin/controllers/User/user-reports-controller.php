...
class UserReportsController extends BaseController {
    public static function listOpen(int $limit = 100): array {
        $sql = "SELECT r.id, r.reported_user_id, r.reporter_user_id, r.reason, r.status, r.created_at, 
                       ru.username AS reported_username,
                       reporter_u.username as reporter_username
                FROM user_reports r 
                JOIN users ru ON ru.id = r.reported_user_id
                JOIN users reporter_u ON reporter_u.id = r.reporter_user_id
