<?php
// AdoptPetProcessController.php - handles adoption requests from public users and status updates
...
class AdoptPetProcessController extends BaseController {
    /**
     * Handle POST request from adoption form or status updates
     */
    public static function handleRequest(): void {
        ...
        // Handle status-change actions (approve/deny/cancel/reopen)
        if (in_array($action, ['approve','deny','cancel','reopen'], true)) {
            // Basic permission check could be added here (shelter owner / admin)
            $adoptionId = isset($_POST['adoption_id']) ? (int)$_POST['adoption_id'] : 0;
            if ($adoptionId <= 0) {
                self::redirectWithError('Invalid adoption request id.');
            }
            ...
            $targetStatus = $map[$action] ?? null;
            if ($targetStatus === null) self::redirectWithError('Unknown action.');

            $mysqli = PetManagementController::db();
            if (!$mysqli) self::redirectWithError('Database connection not available.');

            $stmt = $mysqli->prepare('UPDATE adoptions SET status = ? WHERE id = ? LIMIT 1');
            if (!$stmt) self::redirectWithError('Failed to prepare update statement.');
            $stmt->bind_param('si', $targetStatus, $adoptionId);
            if (!$stmt->execute()) {
                $stmt->close();
                self::redirectWithError('Failed to update adoption request.');
            }
            $affected = $stmt->affected_rows;
            $stmt->close();

            if ($affected > 0) {
                $_SESSION['flash'] = ['type' => 'success', 'message' => 'Adoption request ' . $targetStatus . '.'];
            } else {
                $_SESSION['flash'] = ['type' => 'warning', 'message' => 'No changes made to the adoption request.'];
            }

            $referer = $_SERVER['HTTP_REFERER'] ?? '../views/AdoptionRequestManagement.php';
            header('Location: ' . $referer);
            exit;
        }

        // Existing 'request' submission flow
        if ($action !== 'request') {
